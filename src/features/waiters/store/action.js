...
export const actionSaveItem = (waiter) => {
  return (dispatch) => {
    if (waiter.id) {
        WaitersApi.update(waiter.id, waiter).then((newWaiter) => dispatch(actionUpdateItem(newWaiter)))
    } else {
...
export const actionRemoveItem = (id) => {
  return (dispatch) => {
    TodoApi.delete(id).then(() => dispatch({ type: ACTION_WAITER_REMOVE_ITEM, payload: id }))
  }
};